...
// Class Definitions
class BasicLogin implements LoginStrategy {
  login(credentials: { username: string; password: string }) {
    const isValid = credentials.username === 'fauzia' && credentials.password === 'fauzia@123';
...
class EmailLogin implements LoginStrategy {
  login(credentials: { email: string; password: string }) {
    const isValid = credentials.email === 'davis@gmail.com' && credentials.password === 'davis@123';
...
class Computer implements IComputer {
  ...
  login(credentials: any) {
    const success = this.loginStrategy.login(credentials);
    ...
    // !success && console.log('Login failed');
  }
...
computer = new Computer('XPS-13', 'DFH-BRA-BRA-BRA-1X34', 'DELL', new Mouse(), new Monitor(), new BasicLogin()); //Super
let laptop: Computer = new Laptop('LPS-13', 'RFH-BXA-BRA-BRA-1X34', 'HP', new Keyboard(), new Projector(), new EmailLogin()); // Subtype
let unbootableComputer: Computer = new UnbootableComputer('UPS-13', 'RFH-BXA-BRA-BRA-1X34', 'HP', new USB(), new Monitor(), new BasicLogin()); // Subtype

computer.setInputDevice(new Keyboard());
computer.login({ username: 'fauzia', password: 'fauzia@123' });
